...
flowchart TB
    ...
    subgraph SubAgents["サブエージェントプール"]
        SA1[リサーチエージェント]
        SA2[コードエージェント]
        SA3[執筆エージェント]
        SA4[分析エージェント]
    ...
    subgraph MemoryLayer["メモリー層"]
        STM[短期メモリ<br/>セッションコンテキスト]
        LTM[長期メモリ<br/>ベクターストア]
        PM[手順メモリ<br/>スキルとツール]
    end

    subgraph SandboxLayer["サンドボックス層"]
        FS[ファイルシステム<br/>/mnt/workspace]
        EXEC[Bash 実行<br/>隔離コンテナ]
        NET[ネットワークアクセス<br/>制御付き]
    end

    subgraph ExternalTools["外部ツール"]
        MCP[MCP サーバー]
        WEB[Web 検索/フェッチ]
        CUSTOM[カスタム Python ツール]
    end
...
**アーキテクチャの説明：**
- **ユーザー層**：ユーザーリクエストのエントリーポイント（Web、IM、API、CLI）
- **コアエンジン**：タスクのオーケストレーション、コンテキスト管理、スケジューリングを担当
- **サブエージェントプール**：専門化されたエージェントが具体的なタスクを実行
- **メモリー層**：短期セッションコンテキストと長期ベクターメモリを管理
- **サンドボックス層**：安全なファイル操作とコード実行環境を提供
- **外部ツール**：MCP サービス、ネットワーキングツール、カスタム拡張を統合
